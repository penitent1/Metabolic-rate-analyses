...
#library(rfishbase)
getwd()
knope <- read.nexus("bayes_tree_final_KnopeTree.nex") #keep it as a nexus file inside R 

plot.phylo(knope) #really crowded tree
plot.phylo(knope, cex=0.5) #makes the font smaller, easier to see

is.ultrametric(knope.tree)
is.rooted(knope.tree)
...
knope.tree$tip.label
head(knope.tree$tip.label)
...
depthdata <- species(taxa, fields=c("DepthRangeShallow", "DepthRangeDeep"))
depthdata

## The rOpenSci team has created a pilot API for SeaLifeBase as well. 
## Most of the functions in rfishbase can be used directly to query 
## SeaLifeBase data by explicitly specifying the server argument to use
## the SeaLifeBase API at http://fishbase.ropensci.org/sealifebase, like so:
...
studies_find_trees(property="ot:studyId", value="ot_96")

taxa <- c("Artedius lateralis",
...
## We'll use a dataset of anolis lizards 
tree <- read.tree("anolis.tre.R")
...
## We might want to change the elements of a tree. 
## For example, for some analyses if we are only interested 
## in relative divergence times, we may want to rescale the total 
## tree depth to be one. This allows us to more easily compare 
## the parameters of a trait model across different trees 
## (e.g., the results of a Bayesian analysis)
head(tree$edge.length)
tree$edge.length <- tree$edge.length * 2
head(tree$edge.length)

knope.tree.new <- drop.tip(knope.tree.new, "Stellerina_xyosterna")
