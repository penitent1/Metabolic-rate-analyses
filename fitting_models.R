...
anolis_td <- treedata(anolis_phy, anolis_dat)
str(anolis_td)

dummy_tree <- anolis_phy
head(dummy_tree$tip.label)
dummy_tree_drop <- drop.tip(dummy_tree, tip="ahli")
dummy_tree_drop
...
## Estimating phylogenetic signal using Blomberg's K
## install.packages("picante")
library(picante)
K <- phylosignal(states, tree)
...
## ## Discrete Characters
## Simulate characters on anolis tree because we don't have discrete data available for this group
