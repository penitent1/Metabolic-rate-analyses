...
probe14.arfe <- probe14[probe14$spps == 'arfe', ] # Update as appropriate
probe14.arfe.17jun <- probe14.arfe[probe14.arfe$date.day == '17' &
                                     probe14.arfe$date.month == 'jun' , ] # Update as appropriate
#probe14.olma.trial2.16c <- probe14.olma.trial2[probe14.olma.trial2$temp == '16', ] # Update as appropriate
head(probe14.arfe.17jun) # Update as appropriate
str(probe14.arfe.17jun) # Update as appropriate

### SMR estimate function

five.hr.plus.data <- probe14.arfe.17jun[probe14.arfe.17jun$time.hrs > 5, ]
...
plot.smr <- ggplot(data=probe14.arfe.17jun, aes(x=probe14.arfe.17jun$time.hrs, 
                                                  y=probe14.arfe.17jun$mo2)) 
plot.smr + (geom_point(size = 1.41)) +
  geom_hline(yintercept = 1.41) +
  labs(x = "Time (hrs)",
       y = "MO2 (umol O2/g/hr)") +
  theme_base()
...
calcO2crit(pcrit.data, 1.44) # Enter value of SMR obtained above here, after "pcrit.data
#?calcO2crit

plotO2crit(calcO2crit(pcrit.data, 1.44))
