...
probe13.arfe.11jul <- probe13.arfe[probe13.arfe$date.day == '11' &
                                     probe13.arfe$date.month == 'jul' , ] # Update as appropriate
#probe14.olma.trial2.16c <- probe14.olma.trial2[probe14.olma.trial2$temp == '16', ] # Update as appropriate
head(probe13.arfe.11jul) # Update as appropriate
str(probe13.arfe.11jul) # Update as appropriate

### SMR estimate function

five.hr.plus.data <- probe13.arfe.11jul[probe13.arfe.11jul$time.hrs > 5, ]
...
plot.smr <- ggplot(data=probe13.arfe.11jul, aes(x=probe13.arfe.11jul$time.hrs, 
                                                  y=probe13.arfe.11jul$mo2)) 
plot.smr + (geom_point(size = 1.5)) +
  geom_hline(yintercept = 2.62) +
  labs(x = "Time (hrs)",
       y = "MO2 (umol O2/g/hr)") +
  theme_base()
...
calcO2crit(pcrit.data, 2.62) # Enter value of SMR obtained above here, after "pcrit.data
#?calcO2crit

plotO2crit(calcO2crit(pcrit.data, 2.62))
