...
probe13.arha.4jul <- probe13.arha[probe13.arha$date.day == '4' &
...
head(probe13.arha.4jul) # Update as appropriate
str(probe13.arha.4jul) # Update as appropriate

### SMR estimate function

five.hr.plus.data <- probe13.arha.4jul[probe13.arha.4jul$time.hrs > 5, ]
...
plot.smr <- ggplot(data=probe13.arha.4jul, aes(x=probe13.arha.4jul$time.hrs, 
                                                  y=probe13.arha.4jul$mo2)) 
plot.smr + (geom_point(size = 1.5)) +
  geom_hline(yintercept = 2.23) +
  labs(x = "Time (hrs)",
       y = "MO2 (umol O2/g/hr)") +
  theme_base()
...
calcO2crit(pcrit.data, 2.23) # Enter value of SMR obtained above here, after "pcrit.data
#?calcO2crit

plotO2crit(calcO2crit(pcrit.data, 2.23))
